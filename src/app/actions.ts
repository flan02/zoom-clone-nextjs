...
export async function getToken() {
  ...
  // TODO Create a StreamClient instance
  const streamClient = new StreamClient(streamApiKey, streamApiSecret, {});

  const expirationTime = Math.floor(Date.now() / 1000) + 60 * 60;

  const issuedAt = Math.floor(Date.now() / 1000) - 60; // ! This technique is used to avoid clock skew issues between the client and server
